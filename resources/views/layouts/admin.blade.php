...
<head>
    ...
    <title>@yield('title', 'Panel Administrativo') — {{ $perfilNegocio->tituloWeb() ?? config('rentaltech.empresa.razon_social') }}</title>
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sb-nav">
        @foreach (config('menu.admin') as $seccion => $items)
            @php
                $itemsVisibles = collect($items)->filter(
                    fn ($item) => in_array(auth()->user()->rol, $item['roles'] ?? ['admin'])
                );
            @endphp
