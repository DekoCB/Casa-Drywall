...
test('puede crear guía con vehículos secundarios', function () {
    ...
    $guide = DispatchGuide::first();
    expect($guide->vehiculo['placa_secundaria'])->toHaveCount(2);
    expect($guide->vehiculo['placa_secundaria'])->toBe(['VEH002', 'VEH003']);
});
...
test('puede generar PDF para una guía de remisión', function () {
    ...
    expect($responseData['document_id'])->toBe($dispatchGuide->id);
    expect($responseData['document_type'])->toBe('dispatch-guide');
});
