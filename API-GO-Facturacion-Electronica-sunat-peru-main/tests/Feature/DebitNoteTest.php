...
test('puede generar PDF para una nota de débito', function () {
    ...
    expect($responseData['document_id'])->toBe($debitNote->id);
    expect($responseData['document_type'])->toBe('debit-note');
    expect($responseData['pdf_path'])->toContain($debitNote->serie);
});